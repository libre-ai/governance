...
| Contrôle d'advisories de flotte (ADR-0021 D1)               | planificateur hebdomadaire + dispatch manuel  | historique des runs du workflow, rapport par dépôt   | terminal à chaque run (un passage = un verdict par dépôt vivant)                             | run rouge = advisory sur un dépôt vivant, **ou absence de run récent** `[rattrapable]` | relance manuelle du workflow              |
