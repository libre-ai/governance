...
| Conformité de la recette de preuves (étape du job `Inventory drift`)       | planificateur hebdomadaire + dispatch manuel  | rapport par dépôt (variante attendue, première ligne divergente) ; issue de récidive du workflow                                         | terminal à chaque run (un verdict par dépôt avec manifeste ; archivé ou sans manifeste = exempt affirmé ; injoignable = « unable to verify »)  | dépôt avec `package.json` ou `Cargo.toml` sans copie octet-exacte de sa variante `distribution/templates/evidence/`, **ou absence de run récent** `[rattrapable]` | coller la variante dans `.evidence.json` du dépôt, relancer         |
