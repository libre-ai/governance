...
Add the negative recovery vector: take an execution and tombstone snapshot before deleting A, then present that stale registry after the deletion writer fence. Even though the local join reports zero, count/digest coverage verification must refuse pre-open. Also refuse an execution snapshot older than `P35D`, a missing manifest, wrong count, wrong digest, mismatched execution-snapshot or snapshot-catalog digest, absent/partial retirement proof and coverage ending before the fence. In a real concurrent E2E, capture an execution snapshot between tombstone capture and deletion commit, advance time just beyond the tombstone's `P35D` eligibility, and prove purge plus restore omission both refuse until an authenticated retirement fact excludes that exact snapshot; age alone must stay red.
...
writers_fenced_at`, `observed_at - snapshot_at <= P35D`, the exact execution
snapshot digest, and the snapshot-catalog/retirement proof binding. One
...
authenticated catalog membership and retirement proof bind this exact selected
snapshot; elapsed time is not consulted. The package validates those exact
digest/revision bindings while the future caller authenticates their external
semantics. `suppressed_lineage_count` repeats the registry proof in its own
