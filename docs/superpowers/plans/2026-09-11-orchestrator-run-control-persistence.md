...
For every JSON/JCS file, it must also parse the document and recursively scan every decoded JSON string, including keys, so escape sequences cannot bypass the raw scan. No file, review role, extension or evidence directory receives an exemption. The same pure validator supports a content-only mode over staged Git blobs before `E` is created; the historical evidence gate reruns it over the exact blobs at `E`.
