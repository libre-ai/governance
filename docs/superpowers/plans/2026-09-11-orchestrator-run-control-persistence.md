...
Bootstrap one synthetic database, `pgcrypto`, the four global capability roles and three login identities with every attribute explicit: capability roles use `NOLOGIN NOSUPERUSER NOINHERIT NOCREATEROLE NOCREATEDB NOREPLICATION NOBYPASSRLS CONNECTION LIMIT 0`; login identities use the same negative attributes except `LOGIN`, an explicit positive connection limit and a synthetic fixed expiry. Set no role configuration. Each login identity is a member of exactly its matching app/retention/restore role without admin option, no identity can assume guard, and no capability role has outbound membership. The wrapper branches on `server_version_num`: PostgreSQL 16+ grants the three memberships with `ADMIN FALSE, INHERIT FALSE, SET TRUE`, while 14/15 relies on the identity's explicit `NOINHERIT` and `admin_option = false`; both paths run the real `SET LOCAL ROLE` probes. PostgreSQL 15+ also proves no restricted principal or `PUBLIC` has a parameter ACL. Export percent-encoded Unix-socket app/retention/restore URLs only to the child command after `--`.
