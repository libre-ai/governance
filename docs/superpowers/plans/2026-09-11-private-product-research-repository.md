...
- Create: `docs/adr/0032-private-product-research-repository.md`
...
- Produces local `/Users/ifi6567/Documents/libre-ai/product-research` and remote `libre-ai/product-research`.
...
### Task 8: Import the sanitized commercialization research

**Files:**

- Create: `sessions/2026-09-10-commercialization/synthesis.md`
- Create: `hypotheses/RSH-0001-critical-application-maintenance.md`
- Create: `offers/RSH-0002-sovereign-managed-service.md`
- Create: `offers/RSH-0003-commercial-model.md`
- Create: `market/RSH-0004-regulated-critical-sectors.md`
- Create: `regulatory-mappings/RSH-0005-claims-boundary.md`
- Create: `narratives/RSH-0006-fundraising-narrative.md`
- Create: `decisions/proposed/RSH-0007-open-supported-profile.md`
- Modify: `registry/research-items.v1.json`, `registry/sources.v1.json`, `sources/README.md`

**Interfaces:**

- Produces seven traceable research items; none is `promoted`.
- Consumes only sanitized internal observation and official public sources.

- [ ] **Step 1: Register sources**

`SRC-0001` is `internal-observation` titled `Owner-arbitrated commercialization research session`, observed 2026-09-11; no identity or transcript. Add official HTTPS sources for DORA EU 2022/2554, AI Act EU 2024/1689, CRA EU 2024/2847 and current SGDSN IGI 1300.

- [ ] **Step 2: Write the seven records**

1. `RSH-0001` problem/supported: critical maintenance must be reproducible, auditable and restorable under constrained operation.
2. `RSH-0002` offer/supported: sovereign managed service first; mutualized, dedicated-connected and dedicated-air-gap signed profiles.
3. `RSH-0003` offer/supported: Assurance + client Qualification + modular Operations; price by trust domain and capacity class, never seat/agent/token.
4. `RSH-0004` market/hypothesis: regulated critical domains and actual contracts; no unsupported market-size claim.
5. `RSH-0005` regulatory-mapping/supported: regulations constrain selection/evidence; never automatic compliance or certification.
6. `RSH-0006` narrative/supported: editor-operator of critical application maintenance; no fabricated ROI percentage, only observed accounting substitution.
7. `RSH-0007` decision/supported: copyable by design; official supported profiles differentiated by signed evidence and responsibility; no vendor lock-in.

All are `internal`, cite `SRC-0001` plus applicable primary sources, and set `authority: null`.

- [ ] **Step 3: Verify hostile-content and registry gates**

Run:

```bash
bun scripts/check-research.ts --base-ref origin/main
bun scripts/check-content-safety.ts
bun scripts/check-authorities.ts
bun run check
```

Expected: seven items, zero safety finding, zero promoted reference, no raw transcript or personal identifier.

- [ ] **Step 4: Commit, review and merge**

Commit `docs: preserve commercialization product research` with DCO sign-off. Create a PR carrying the owner-arbitration marker, perform adversarial review and merge only after required checks pass.
